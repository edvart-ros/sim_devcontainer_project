...
class ObstacleCourse:
    ...
    def boatSector(self,other_boat):
        currentPos = self.boat.pos
        currentHeading = currentPos.heading
        buoyDict = self.boat.getBuoyDict()

        if BOAT in buoyDict.keys():
            for i in range(len(buoyDict[BOAT])):
                boat_detection_pos = buoyDict[BOAT][i].pos 
                x_pos = boat_detection_pos.getX()
                y_pos = boat_detection_pos.getY()
                vector_x = x_pos - currentPos.getX()

        return "" #left, right and center
